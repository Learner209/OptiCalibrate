...
class CircleGrid(BaseMarker):
    ...
    def estimate_pose(self, corners: np.ndarray, ids: np.ndarray,
        ...
        if ret:
            ...
            proj_points, _ = cv2.projectPoints(
                self.obj_points, rvec, tvec, camera_matrix, dist_coeffs)
            self.rep_error = np.sqrt(np.mean((corners.reshape(-1, 2) - 
                                            proj_points.reshape(-1, 2)) ** 2))
    ...
    def generate_pattern(self, size: Tuple[int, int] = (1000, 1000)) -> np.ndarray:
        ...
        spacing_pixels = min(total_width / (self.pattern_size[1] - 1),
