...
class CalibrationDataCollector:
    ...
    async def collect_data(self) -> Tuple[List[np.ndarray], List[Tuple[np.ndarray, np.ndarray]]]:
        """
        异步采集标定数据
        """
        logger.info("开始采集数据")
        ...
        while self.running:
            ...
            robot_pose = await asyncio.get_event_loop().run_in_executor(
                None, self.robot.get_current_pose)
            ...
            corners, ids, rejected = await asyncio.get_event_loop().run_in_executor(
                None, self.marker_detector.detect, color_frame)
            if corners is not None and len(corners) > 0:
                # 估计标记位姿
                rvecs, tvecs, rep_error = await asyncio.get_event_loop().run_in_executor(
                    None, 
